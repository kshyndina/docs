...
Steamboat is Triton's account indexing engine that delivers `getProgramAccounts`, `getTokenAccountsByOwner`, and `getTokenAccountsByDelegate` up to **50x faster** by serving them from tailored PostgreSQL indexes instead of full account scans. Indexes are built automatically from your query traffic. Available on every Triton plan at standard RPC rates -- no premium for indexed reads. See [Steamboat](../documentation/reading-state/steamboat-indexed-accounts.md).
...
Fumarole is a streaming system designed for reliable, scalable streaming of account and transaction data. It aggregates data from multiple Solana nodes for high availability and persistence, with up to 4 days of retained stream history. Available on every Triton subscription, with regional endpoints (`ams.rpcpool.com` for Europe, `nyc.rpcpool.com` for the US). See [Fumarole](../documentation/streaming-data/fumarole-persistent-streams.md).
...
No, gRPC is not supported in web browsers due to HTTP/2 and protobuf limitations. It is designed for backend applications. For frontend use, use [Whirligig WebSockets](../documentation/streaming-data/whirligig-websockets.md) instead.
...
Replace `https` with `wss` in your RPC endpoint URL. For example: `wss://your-endpoint.rpcpool.com`. For backends, we recommend using gRPC instead -- see [Dragon's Mouth gRPC](../documentation/streaming-data/dragon-s-mouth-grpc.md) for the lower-latency path.
...
Full guide: [Verify your gRPC endpoint](../guides/error-handling/verify-your-grpc-endpoint.md).
