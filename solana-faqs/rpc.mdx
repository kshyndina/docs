---
title: "RPC requests"
description: "Questions about Triton's RPC services, latency, rate limits, scripts and monitoring."
---
...
<AccordionGroup>
  ...
  <Accordion title="What happens if I hit my rate limit?">
    Your application will receive `HTTP 429` errors. When this happens, pause requests for 10 seconds to clear the limit. We strongly recommend implementing a backoff-and-retry mechanism. See [Handle 429 rate-limit errors](/solana-guides/error-handling/handle-429-rate-limits) for the pattern, or our [Rate and connection limits](/solana/get-started/rate-and-connection-limits) page for the budgets and headers.
  </Accordion>

  <Accordion title="Can I run scripts on the shared service?">
Yes. Shared infrastructure handles backend workloads (scripts, trading bots, indexers) and is sized to absorb traffic spikes. If you expect sustained heavy load (heavy subscriptions, full-chain streams, custom Geyser configurations), a [dedicated node](/solana/dedicated-nodes/overview) is usually the right choice.
  </Accordion>
</AccordionGroup>
...
Customers with [dedicated nodes](/solana/dedicated-nodes/overview) get a Grafana dashboard with real-time metrics: request volume, latency, error rates, and bandwidth usage.
  ...
  <Accordion title="Can I create new tokens or endpoints myself?">
  ...
  <Accordion title="What's the __blocked.rpcpool.com origin on my endpoint?">
  ...
  <Accordion title="How does Triton secure dedicated node endpoints?">
    Dedicated node endpoints use:

    - **API tokens:** unique per client, revocable from the [customer dashboard](https://customers.triton.one).
  ...
  <Accordion title="Can I use Jito sendBundle and bundle simulation with Triton RPCs?">
    Bundle simulation is available to everyone -- see [Jito bundles](/solana/sending-transactions/jito-bundles) for the full reference.
...
<AccordionGroup>
  <Accordion title="I have an issue with my Web3.js socket connection. How can I fix it?">
    If you're hitting persistent socket or connection errors with `@solana/web3.js` (`fetch failed`, `Connect Timeout Error`, `ECONNREFUSED`, `ECONNRESET`, `other side closed`), see [Web3.js socket and connection issues](/solana-guides/error-handling/web3js-connection-issues). It covers the root causes and the standard fixes.
  </Accordion>
