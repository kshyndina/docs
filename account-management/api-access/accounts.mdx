...
Returns a hash with the `account` key containing an account object. The response data for the account object is the same as for `GET /api/v1/accounts`. Below is the sample response.
...
Returns a hash with the `account` key containing an account object. The response data for account object is the same as for `GET /api/v1/accounts/:account-uuid` nad `GET /api/v1/accounts`
