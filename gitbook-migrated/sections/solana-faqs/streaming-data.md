...
Steamboat is Triton's account indexing engine that delivers `getProgramAccounts`, `getTokenAccountsByOwner`, and `getTokenAccountsByDelegate` up to **50x faster** by serving them from tailored PostgreSQL indexes instead of full account scans. Indexes are built automatically from your query traffic. Available on every Triton plan at standard RPC rates -- no premium for indexed reads. See [Steamboat](../solana-documentation/reading-state/steamboat-indexed-accounts.md).
...
No, gRPC is not supported in web browsers due to HTTP/2 and protobuf limitations. It is designed for backend applications. For frontend use, use [Whirligig WebSockets](../solana-documentation/streaming-data/whirligig-websockets.md) instead.
...
Yes, on both shared and dedicated infrastructure. Your existing WebSocket connections automatically route through [Whirligig WebSockets](../solana-documentation/streaming-data/whirligig-websockets.md) -- our gRPC-backed WebSocket layer that delivers intra-slot account updates at `processed`, full `blockSubscribe` and `transactionSubscribe`, and significantly higher subscription limits than native pubsub. No code changes required.
...
Replace `https` with `wss` in your RPC endpoint URL. For example: `wss://your-endpoint.rpcpool.com`. For backends, we recommend using gRPC instead -- see [Dragon's Mouth gRPC](../solana-documentation/streaming-data/dragon-s-mouth-grpc.md) for the lower-latency path.
...
Full guide: [Verify your gRPC endpoint](../solana-guides/error-handling/verify-your-grpc-endpoint.md).
