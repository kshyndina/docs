...
Your application will receive `HTTP 429` errors. When this happens, pause requests for 10 seconds to clear the limit. We strongly recommend implementing a backoff-and-retry mechanism. See [Handle 429 rate-limit errors](../solana-guides/error-handling/handle-429-rate-limit-errors.md) for the pattern, or our [Rate and connection limits](../get-started/get-started/rate-and-connection-limits.md) page for the budgets and headers.
...
Yes. Shared infrastructure handles backend workloads (scripts, trading bots, indexers) and is sized to absorb traffic spikes. If you expect sustained heavy load (heavy subscriptions, full-chain streams, custom Geyser configurations), a [dedicated node](../solana-documentation/dedicated-nodes/overview.md) is usually the right choice.
...
Customers with [dedicated nodes](../solana-documentation/dedicated-nodes/overview.md) get a Grafana dashboard with real-time metrics: request volume, latency, error rates, and bandwidth usage.
...
Bundle simulation is available to everyone -- see [Jito bundles](../solana-documentation/sending-transactions/3rd-party-apis/jito-bundles.md) for the full reference.
...
If you're hitting persistent socket or connection errors with `@solana/web3.js` (`fetch failed`, `Connect Timeout Error`, `ECONNREFUSED`, `ECONNRESET`, `other side closed`), see [Web3.js socket and connection issues](../solana-guides/error-handling/web3js-socket-connection-issues.md). It covers the root causes and the standard fixes.
