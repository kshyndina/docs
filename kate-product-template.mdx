...
description: "Reference layout for a single Triton product. Title, value prop, what-it-is, why-it-works, how-it-works, get-started CTA."
...
- If a product has multiple sub-features (e.g. Yellowstone has Dragon's Mouth, Whirligig, Fumarole, Hermes), make each its own product page and use the [overview template](/kate-overview-template) for the parent.
...
<CardGroup cols={2}>
  <Card title="Quickstart: your first Steamboat index" icon="rocket" href="#">
    Define a filter, point it at a program, and read it back in under five minutes.
  </Card>
  <Card title="Available endpoints" icon="server" href="#">
    HTTPS REST and gRPC reads, JS / Python / Rust SDKs.
  </Card>
</CardGroup>
...
Included on all PAYG and dedicated plans. Reads are billed at `$10 / million calls + bandwidth` (same as Standard RPC). See the [pricing calculator](/solana/get-started/plans-and-billing).
...
<AccordionGroup>
  <Accordion title="Does Steamboat support all Solana programs?">
...
<CardGroup cols={2}>
  <Card title="Dragon's Mouth gRPC" icon="radio" href="/solana/streaming/dragons-mouth/overview">
    The streaming layer Steamboat builds on. Subscribe to live account writes.
  </Card>
  <Card title="Standard RPC" icon="zap" href="/solana/reading-state/standard-rpc">
    The simpler `getAccountInfo` / `getProgramAccounts` path for low-volume reads.
  </Card>
</CardGroup>
