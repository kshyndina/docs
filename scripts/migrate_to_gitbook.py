...
def convert_blocks(text, ctx):
    ...
    # param / response fields -> list items
    def paramfield(m):
        tag, body = m.group(1), (m.group(2) or "").strip()
        name = attr(tag, "name") or attr(tag, "path") or attr(tag, "query") or attr(tag, "body") or ""
        typ = attr(tag, "type")
        req = "required" in tag and "required={false}" not in tag and "required=\"false\"" not in tag
        head = f"**`{name}`**"
        if typ:
            head += f" `{typ}`"
        if req:
    ...
    def paramfield_sc(m):
        ...
        name = attr(tag, "name") or attr(tag, "path") or attr(tag, "query") or attr(tag, "body") or ""
        typ = attr(tag, "type")
        req = "required" in tag and "required={false}" not in tag
        head = f"**`{name}`**"
        if typ:
            head += f" `{typ}`"
        if req:
            head += " _(required)_"
        return f"- {head}\n"
    text = re.sub(r"<ParamField\b([^>]*)>(.*?)</ParamField>", paramfield, text, flags=re.S)
    text = re.sub(r"<ParamField\b([^>]*)/>", paramfield_sc, text)
    text = re.sub(r"<ResponseField\b([^>]*)>(.*?)</ResponseField>", paramfield, text, flags=re.S)
    text = re.sub(r"<ResponseField\b([^>]*)/>", paramfield_sc, text)

    # tree -> code block
    def tree(m):
        return "\n```\n" + re.sub(r"</?\w+[^>]*>", "", m.group(1)).strip() + "\n```\n"
...
def inline_snippets(text):
    ...
    for var, path in imports.items():
        ...
        text = re.sub(rf"<{var}>\s*</{var}>", lambda m, s=snippet: s, text)
    return text
