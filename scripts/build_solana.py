#!/usr/bin/env python3
"""Solana-default experiment site: duplicate of the migrated content, synced from
the isolated `solana-experiment` branch (Solana headline + on-load hints + Solana
API reference label). Other sites are untouched. Tabs renamed to foreground Solana."""
...
# (section key, tab title) -- Solana tabs labelled to make the default obvious
SECTS = [("solana-documentation", "Solana docs"), ("solana-guides", "Solana guides"),
         ("solana-api-reference", "Solana API reference"), ("solana-faqs", "FAQs"),
         ("sui", "Sui"), ("monad", "Monad")]
...
for key, title in SECTS:
    st, s = call("POST", f"/orgs/{ORG}/spaces", {"title": f"solana {title}"})
    sp[key] = s["id"]
...
st, site = call("POST", f"/orgs/{ORG}/sites", {"title": "Triton One Docs", "type": "basic"})
SITE = site["id"]
call("PATCH", f"/orgs/{ORG}/sites/{SITE}", {"basename": "triton-docs-solana"})
...
print("SOLANA EXPERIMENT URL:", s2.get("urls", {}).get("published"))
json.dump({"site": SITE, "spaces": sp}, open("/tmp/gb_solana.json", "w"))
