...
gRPC is unsupported by web browsers, so Dragon's Mouth is entirely targeted at backend software. Another Yellowstone project, [Whirligig](https://docs.triton.one/project-yellowstone/whirligig-websockets), provides a WebSocket interface to replace the current Solana WebSocket implementation.
...
This will return all the bs as they are produced. It will send bs along with the transactions:
...
If you want to subscribe just to notifications as bs are processed without receiving all the transactions, then you can use the b meta subscription:
...
**Example: ing replay availability**
...
You then also make sure you subscribe to [slot notifications](https://docs.triton.one/project-yellowstone/dragons-mouth-grpc-subscriptions#subscribe-to-slots). This will give you rmation about when a slot is confirmed or finalized. Depending on the commitment level you are interested in, you should release your buffer when you receive the slot notification for a particular slot at a particular commitment level.
...
This is the earliest usable on-chain signal exposed by Dragon's Mouth. It is designed for latency-sensitive systems that care about transaction intent as early as possible, such as arbitrage, market making, trading, liquidations, and HFT pipelines.
