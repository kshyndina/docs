...
<AccordionGroup>
  ...
  <Accordion title="Can I use the token client-side, in browser code?">
    No. The token is a server-side secret. For browser apps, set up an **origin allowlist** in the dashboard so requests are authorised by the page origin, not the token. See [Auth and security](/solana/get-started/auth-and-security).
  </Accordion>
...
<AccordionGroup>
  <Accordion title="I'm getting 401 Unauthorized">
    ...
    2. The request origin isn't on the allowlist. See [Auth and security](/solana/get-started/auth-and-security).
