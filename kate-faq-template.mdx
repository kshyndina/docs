...
<AccordionGroup>
  ...
  <Accordion title="Why is `getProgramAccounts` slow at scale?">
    The native Solana RPC scans the validator's account index linearly. For programs with >100k accounts that's tens of seconds. Use [Steamboat](/solana/reading-state/steamboat-indexed-accounts) for a hot index keyed exactly on the lookup pattern your app uses; reads return in milliseconds.
  </Accordion>
</AccordionGroup>
