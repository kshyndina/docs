---
title: "Triton RPC error codes"
description: "Possible RPC symptoms and the full list of Triton-specific RPC error codes for HTTP, WebSocket, and gRPC services."
sidebarTitle: "Triton RPC error codes"
---
...
### Private pool users see "Access denied" (403) despite valid API keys and IPs
...
| You've hit the custom rate limit set for your private pool plan, or a misconfigured client is overloading the endpoint. Slow down your request frequency or upgrade to a private pool with higher limits. | 429 | Client |
...
| The client does not have enough memory to hold the server response. Implement exponential backoff and reduce request frequency. Close unused WebSocket subscriptions. Upgrade your private pool for higher limits. | RESOURCE\_EXHAUSTED | Client |
