...
<Tabs>
  ...
  <Tab title="Old Faithful streams">
    **What we're doing:** connect to Old Faithful's gRPC server and call `stream_blocks` to replay a range of historical blocks (slots 100,000,000 to 100,000,100). Old Faithful's streaming methods are `StreamBlocks` and `StreamTransactions` -- both replay historical data with server-side filters.
...
3. **Widen the filter to test, then tighten back.** Subscribe to all transactions or all writes for a busy program (e.g. SPL Token) to confirm data is reaching you. Once you see traffic, narrow the filter to what your app actually needs.
