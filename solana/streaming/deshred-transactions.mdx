...
<ParamField path="account_include" type="string[]">
  Accounts mentioned anywhere in the transaction (including loaded ALT addresses).
</ParamField>
...
For a deeper overview of architecture and tradeoffs, see [Deshred transactions: the fastest path to Solana data](https://blog.triton.one/deshred-transactions-the-fastest-path-to-solana-data/). For the wire format and proto definitions, see [`geyser.proto`](https://github.com/rpcpool/yellowstone-grpc/blob/master/yellowstone-grpc-proto/proto/geyser.proto).
