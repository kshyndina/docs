...
description: "Better, faster Solana WebSockets. Whirligig is a Rust-based proxy between Dragon's Mouth gRPC and a WebSocket client, with intra-slot updates at processed commitment."
---
...
import WgWebSocketCompatibleCard from '/snippets/cards/features/wg-websocket-compatible.mdx'
import WgTransactionSubscribeCard from '/snippets/cards/features/wg-transaction-subscribe.mdx'
import WgCommitmentLevelsCard from '/snippets/cards/features/wg-commitment-levels.mdx'
import WgReliabilityLimitsCard from '/snippets/cards/features/wg-reliability-limits.mdx'
...
For a backend that can speak gRPC, [Dragon's Mouth gRPC](/solana/streaming/dragons-mouth-g-rpc) is the lower-latency path. Whirligig exists for browsers and any client that can't use gRPC.

## Features and benefits

<CardGroup cols={2}>
  <WgWebSocketCompatibleCard />

  <WgTransactionSubscribeCard />

  <WgCommitmentLevelsCard />

  <WgReliabilityLimitsCard />
</CardGroup>
...
Whirligig supports all three commitment levels: `processed`, `confirmed`, `finalized`. You can also [manage commitment levels](/solana/streaming/dragons-mouth-g-rpc#commitment-levels) in your app, the same way as for Dragon's Mouth gRPC. Each subscribe method has a corresponding unsubscribe method that takes the subscription id returned by `subscribe`.
