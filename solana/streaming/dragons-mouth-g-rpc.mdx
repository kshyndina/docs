...
description: "The canonical Yellowstone gRPC stream for Solana. Sub-slot account, transaction, slot, and block updates with fine-grained server-side filters."
sidebarTitle: "Dragon's Mouth"
---
...
**Streaming subscriptions** (open a subscribe stream, receive matching events):
...
| Deshred transactions (paid beta) | Transactions reconstructed from shreds *before* execution. Separate `SubscribeDeshred` method. See [Deshred transactions](#deshred-transactions) |
...
**Unary operations** (one request, one response):
...
- **Within an array, OR.** `account_include: [A, B]` matches transactions mentioning A *or* B.
...
<AccordionGroup>
  <Accordion title="Account filters">
    Match account writes by pubkey, owner program, or content-based filters.

    | Field | Type | Purpose |
    | --- | --- | --- |
    | `account` | `string[]` | List of pubkeys to subscribe to directly |
    | `owner` | `string[]` | List of program owners (subscribes to every account owned by these programs) |
    | `filters` | `AccountsFilter[]` | Additional content-based filters: `memcmp`, `datasize`, `tokenAccountState`, `lamports` |

    See [account subscriptions](#accounts) for examples.
  </Accordion>

  <Accordion title="Transaction filters">
    Match transactions by participating accounts, signature, vote/failed flags.
    ...
    | `account_required` | `string[]` | Accounts that MUST all be mentioned (every one of them) |

    See [transaction subscriptions](#transactions) for examples.
  </Accordion>

  <Accordion title="Block filters">
    Match block updates and control payload size.

    | Field | Type | Purpose |
    | --- | --- | --- |
    | `account_include` | `string[]` | Only deliver transactions/accounts in the block that mention these |
    | `include_transactions` | `bool` | Include the block's transactions. Default `true`. |
    | `include_accounts` | `bool` | Include accounts updated during the block. Default `false`. |
    | `include_entries` | `bool` | Include the block's entries |

    See [block subscriptions](#blocks) for examples.
  </Accordion>

  <Accordion title="Slot filters">
    Match slot-status updates.

    | Field | Type | Purpose |
    | --- | --- | --- |
    | `filter_by_commitment` | `bool` | If `true`, only receive updates matching the request's `commitment`. If `false` (default), receive every status change |

    See [slot subscriptions](#slots) for examples.
  </Accordion>

  <Accordion title="Entry filters">
    No filter parameters. Subscribe by adding any named entry to the `entry` map.

    See [entry subscriptions](#entries) for examples.
  </Accordion>
</AccordionGroup>
...
4. You'll always receive the slot's events *before* its commitment notification.
...
Dragon's Mouth also supports `SubscribeDeshred`, a separate gRPC method that delivers transactions reconstructed from shreds *before* the validator executes them. This is the earliest usable on-chain signal -- designed for arbitrage, market making, copy trading, liquidations, and HFT pipelines.
