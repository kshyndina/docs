...
You send it once when you open the stream; then you can send it again at any time to modify it (the new request fully replaces the old one). **If you're tracking large account sets, [compressed account filters](#compressed-account-filters) reduce payload size ~10x.**
