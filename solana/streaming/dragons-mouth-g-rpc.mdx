...
It's the fastest live data path available and the recommended choice for any backend service where latency matters. For browser clients that can't communicate with gRPC, use [Whirligig WebSockets](/solana/streaming/whirligig-websockets) instead.
...
<CardGroup cols={2}>
  ...
  <Card title="Server-side filtering" icon="server">
    Filter by pubkey, program owner, signature, memcmp, datasize, or token-account state. 
  </Card>

  <Card title="Minimal serialisation overhead" icon="weight">
...
Dragon's Mouth exposes two interfaces on the same gRPC service: streaming subscriptions and one-shot unary calls.Dragon's Mouth exposes two interfaces on the same gRPC service: streaming subscriptions and one-shot unary calls.
...
<Tabs>
  <Tab title="Streaming subscriptions">
    ...
    | Deshred transactions (paid beta) | Transactions reconstructed from shreds _before_ execution. Separate `SubscribeDeshred` method. See [Deshred transactions](https://tritonone.mintlify.app/#deshred-transactions). |
...
#### Account filtersAccount filters
...
### Filter logicFilter logic

Within a single filter category (e.g. one transaction filter), values are combined as follows:Within a single filter category (e.g. one transaction filter), values are combined as follows:
...
[Each transaction update is a `SubscribeUpdateTransaction` containing the full transaction info plus its execution metadata.](#clients-and-sdks)
...
[This is a low-level signal, mostly useful for validator tooling. Most consumers want transactions or blocks instead.](#intra-slot-updates)
...
[To discover the earliest replayable slot, call the unary `subscribeReplayInfo` RPC:](https://gist.github.com/lvboudre/7bbcd895ab3b7df3cd6b0ad1450fac88)
...
[Most consumers of this data want the **end result** processed by the ](https://blog.triton.one/deshred-transactions-the-fastest-path-to-solana-data/)[Photon indexer](https://github.com/helius-labs/photon), not raw account blobs. Triton offers a [hosted Photon service](https://docs.triton.one/chains/solana/photon) -- use that instead.
...
<Tabs>
  ...
  <Tab title="client-ubuntu">
    Prebuilt CLI binary for Ubuntu 22.04 and 24.04. Download from the [releases page](https://github.com/rpcpool/yellowstone-grpc/releases).
