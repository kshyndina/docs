...
description: "Manage account watch lists used by gRPC and WebSocket subscriptions via the Account Management API."
---
...
In the gRPC subscribe message, set `accounts.{filter}.account_filter_name` to the list name instead of inlining the address array. See the [Dragon's Mouth quickstart](/solana/streaming/dragons-mouth-g-rpc) for an example.
...
<Columns cols={2}>
  ...
  <Card title="Dragon's Mouth quickstart" icon="zap" href="/solana/streaming/dragons-mouth-g-rpc">
