...
description: "Tour of the Triton customer portal: organisations, endpoints, tokens, billing, members, and audit log."
keywords: ["triton portal", "customer dashboard", "endpoint management", "organisation", "team members"]
---
...
<Columns cols={2}>
  <Card title="Endpoints" icon="link">
    Provision, monitor, and configure every endpoint your organisation has -- shared, committed, and dedicated.
  </Card>
  ...
  <Card title="Billing" icon="credit-card">
    Plans, payment methods, invoices, and credit balance.
  </Card>
...
- **Endpoints** belong to the organisation. Each has its own URL, region, plan, and limits.
...
| Lower / raise rate limit (committed and above) | **Endpoints → \<endpoint\> → Limits** | 30s |
...
1. **Overview** -- region, plan, status, recent uptime.
...
5. **Limits** -- rate, concurrency, per-method overrides (read-only on PAYG; editable on committed and above).
