...
```mermaid
%%{init: {'theme':'base','themeVariables':{'fontFamily':'Work Sans','fontSize':'13px','primaryColor':'#7A4BA0','primaryBorderColor':'#492D60','primaryTextColor':'#FFFFFF','lineColor':'#956FB3','tertiaryColor':'#F2EDF6'},'flowchart':{'nodeSpacing':22,'rankSpacing':38,'padding':8,'curve':'basis'}}}%%
flowchart TB
  SHARED["Shared"]
  DEDI["Dedicated"]
  STAKE["Staking"]

  SHARED --- READ["Reading state"]
  SHARED --- STREAM["Streaming"]
  SHARED --- HIST["History"]
  SHARED --- SEND["Sending txs"]

  subgraph READ_LEAVES [ ]
    direction TB
    R1["Standard RPC"]
    R2["Steamboat"]
    R3["DAS API"]
  ...
  subgraph STREAM_LEAVES [ ]
    direction TB
    S1["Dragon's Mouth gRPC"]
    S2["Whirligig"]
    S3["Fumarole"]
    S4["Hermes"]
    S5["Pythnet"]
  ...
  subgraph SEND_LEAVES [ ]
    direction TB
    T1["Yellowstone Jet"]
    T2["Priority Fees API"]
    T3["Metis"]
    T4["Titan Prime"]
    T5["Jito Bundles"]
    T1 ~~~ T2 ~~~ T3 ~~~ T4 ~~~ T5
  ...
  subgraph DEDI_LEAVES [ ]
    direction TB
    D1["Dedicated gRPC node"]
    D2["White-label validator"]
    D1 ~~~ D2
  end
  DEDI --- D1 & D2

  STAKE --- K1["Private trusted validator"]

  classDef pillar fill:#7A4BA0,stroke:#492D60,stroke-width:1.5px,color:#FFFFFF,font-weight:700;
  classDef branch fill:#E0D2EC,stroke:#7A4BA0,stroke-width:1.5px,color:#492D60,font-weight:600;
  classDef leaf fill:#F7F2FB,stroke:#C8B4DC,stroke-width:1px,color:#171717,font-weight:500;

  class SHARED,DEDI,STAKE pillar;
  class READ,STREAM,HIST,SEND branch;
  class R1,R2,R3,R4,R5,S1,S2,S3,S4,S5,H1,H2,H3,T1,T2,T3,T4,T5,D1,D2,K1 leaf;
```
