...
sequenceDiagram
    ...
    participant RPC as Triton RPC
    participant Net as Solana network
    Note over You,Net: Read path
    You->>RPC: getBalance, getAccount, getSlot, ...
    Net->>RPC: every block replayed locally (Turbine + ledger)
    RPC-->>You: response from local state
    Note over You,Net: Send path
    You->>RPC: sendTransaction
    RPC->>Net: route to current slot leader TPU<br/>(SWQoS, QUIC)
    Net-->>RPC: leader includes tx in block
    RPC-->>You: signature confirmed
```
