...
| Network | HTTPS RPC | WebSockets | Yellowstone gRPC | Steamboat indexed queries | Historical archive |
|---|:---:|:---:|:---:|:---:|:---:|
...
flowchart TB
    SHARED["Shared infrastructure"] --- READ["Reading state"] & STREAM["Streaming"] & HIST["History"] & SEND["Sending txs"]

    R1["Standard RPC"] ~~~ R2["Steamboat"] ~~~ R3["DAS API"] ~~~ R4["ZK Compression"] ~~~ R5["Account Sync"]
    READ --- R1 & R2 & R3 & R4 & R5

    S1["Dragon's Mouth gRPC"] ~~~ S2["Whirligig"] ~~~ S3["Fumarole"] ~~~ S4["Hermes"] ~~~ S5["Pythnet"]
    STREAM --- S1 & S2 & S3 & S4 & S5

    H1["Hydrant"] ~~~ H2["Old Faithful"] ~~~ H3["Faithful Streams"]
    HIST --- H1 & H2 & H3

    T1["Yellowstone Jet"] ~~~ T2["Priority Fees API"] ~~~ T3["Metis"] ~~~ T4["Titan Prime"] ~~~ T5["Jito Bundles"]
    SEND --- T1 & T2 & T3 & T4 & T5

    DEDI["Dedicated gRPC node"] --- DG["Dragon's Mouth gRPC"]

    VAL["Validator services"] --- V1["White-label validator"] & V2["Private trusted validator"]

    classDef pillar fill:#7A4BA0,stroke:#492D60,stroke-width:1.5px,color:#FFFFFF,font-weight:700,rx:6,ry:6
    classDef branch fill:#E0D2EC,stroke:#7A4BA0,stroke-width:1.5px,color:#492D60,font-weight:600,rx:6,ry:6
    classDef leaf fill:#F7F2FB,stroke:#C8B4DC,stroke-width:1px,color:#171717,font-weight:500,rx:4,ry:4

    class SHARED,DEDI,VAL pillar
    class READ,STREAM,HIST,SEND branch
    class R1,R2,R3,R4,R5,S1,S2,S3,S4,S5,H1,H2,H3,T1,T2,T3,T4,T5,DG,V1,V2 leaf
```
